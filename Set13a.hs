...
split x = help "" x
  where
    help:: String->String->Maybe (String,String)
    help _ "" = Nothing
    help m (x:xs)=if isSpace x then Just ((reverse m),xs) else help (x:m) xs
...
checkNumber (x,y) = if not (null $ filter isDigit x) || not (null $ filter isDigit y) then Nothing else Just (x,y)
...
balance accountName = BankOp f
  where
    f:: Bank->(Int,Bank)
    f (Bank x) = if Map.lookup accountName x == Nothing then (0,Bank x) else let Just xs = Map.lookup accountName x in (xs,Bank x)
...
occurrences xs = state comp
  where
    comp zs = (length (snd $ func zs),snd $ func zs)
      where
        func ys = help xs ys
        help [] ys = ((),ys)
        help (cur:next) ys = help next (count cur ys)
          where
            count x [] = [(x,1)]
            count x ((y,n):ys)
              | x==y = (y, n+1):ys
              | otherwise = (y,n):count x ys
