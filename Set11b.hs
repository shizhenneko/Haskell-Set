...
hSelectLines h nums = mapM (readLine h) nums
    where
        readLine :: Handle->Int->IO String
        readLine h 1=do
            x<-hGetPosn h
            line<-hGetLine h
            hSetPosn x
...
interact' f state = do
    prompt <- getLine
    let (x,y,z)=f (prompt,state)
    putStrLn y
    if x then interact' f z else return z
