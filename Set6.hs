...
instance Eq Name where
  ...
  (==) _ (Name "") = False
  (==)  (Name "") _ = False
  (==) (Name (x:xs)) (Name (y:ys)) = if (Data.Char.toLower x)==(Data.Char.toLower y) then (==) (Name xs) (Name ys) else False
...
instance Eq RationalNumber where
  (==) (RationalNumber x y) (RationalNumber a b) = if b*x==a*y then True else False
...
simplify (RationalNumber x y)= RationalNumber (x `div` val) (y `div` val)
  where
    val = (gcd x y)
...
instance Num RationalNumber where
  (RationalNumber x y) + (RationalNumber a b) = simplify (RationalNumber (b*x + a*y) (b*y))
  (RationalNumber x y) * (RationalNumber a b) = simplify (RationalNumber (a*x) (b*y))
  abs (RationalNumber x y) = simplify (RationalNumber (abs x) (abs y))
  signum (RationalNumber x y) = RationalNumber (helper x) (helper y)
    where 
...
class Cycle a where
  ...
  stepMany 0 x = x
  stepMany 1 x = step x
  stepMany m x = stepMany (m-1) (step x)
