...
statements str_1 str_2 = (liftA2 help str_1 str_2)++(liftA2 help' str_1 str_2)
  where
...
calculator op num =(help op)<*>(readMaybe num)
  where
    help :: String->Maybe (Int->Int)
    help "negate" = Just (\x -> -x)
    help "double" = Just (\x -> 2*x)
    help _ = Nothing
...
validateAddress streetName streetNumber postCode = check (length streetName<=20) "Invalid street name" value *>check (help streetNumber) "Invalid street number" value *> check (length postCode==5 && help postCode) "Invalid postcode" value
  where 
    value::Address
    value=Address streetName streetNumber postCode
    help :: String->Bool
    help "" = True
    help (x:xs) = if isDigit x then help xs else False
...
boolOrInt str = check (isJust (readMaybe str :: Maybe Bool)) "Not a Bool" (Left (unpack (readMaybe str :: Maybe Bool))) <|> check (isJust (readMaybe str :: Maybe Int))  "Not an Int"  (Right (unpack (readMaybe str :: Maybe Int)))
  where
    unpack (Just x) = x
------------------------------------------------------------------------------
...
normalizePhone str = (check (length (strip str)<=10) "Too long" (strip str))*>(traverse checkdigit (strip str))
  where
    strip::String -> String
    strip "" = ""
...
parseExpression str = if length parse /=3 then invalid ("Invalid expression: "++str)
    else if (parse !! 1)=="+" 
      then liftA2 Plus (checkNumber (parse !! 0)) (checkNumber (parse !! 2))
      else if (parse !! 1) == "-"
                then liftA2 Minus (checkNumber (parse !! 0)) (checkNumber (parse !! 2))
                else invalid ("Unknown operator: "++(parse !! 1))*>(lift<$>(checkNumber (parse !! 0) *>(checkNumber (parse !! 2))))
  where
    parse = words str
    checkNumber str
      | isJust (readMaybe str::Maybe Int) = check True "" (Number (read str::Int))
      | isAlpha (head str) && (tail str)=="" = check True "" (Variable (head str))
      | otherwise = check False ("Invalid number: "++str) (Number 0) <|> check False ("Invalid variable: "++str) (Number 0)
    lift :: Arg->Expression
    lift x = Plus x x
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap func (Both fg) = Both (fmap (fmap func) fg)
