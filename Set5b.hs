...
treeMax Empty = 0
treeMax (Node t l r) = maximum [t,treeMax l,treeMax r]
...
allValues condition (Node t l r)= if condition t then (allValues condition l) && (allValues condition r) else False
------------------------------------------------------------------------------
...
isOrdered (Node t l r) 
  | judgel t (valAtRoot l) && judger t (valAtRoot r) = isOrdered l && isOrdered r
  | otherwise = False
  where 
    judger ::Ord a=> a->Maybe a-> Bool
    judger a Nothing = True
    judger a (Just b) = if (a <= b) then True else False

    judgel ::Ord a=> a->Maybe a->Bool
    judgel a Nothing = True
    judgel a (Just b) = if (a >= b) then True else False
...
walk (x:xs) t = walk xs (help x t)
  where 
    help :: Step -> Tree a -> Tree a
    help _ Empty = Empty
    help StepL (Node a l r) = l
    help StepR (Node a l r) = r
------------------------------------------------------------------------------
...
set :: Eq a=>[Step] -> a -> Tree a -> Tree a
set _ _ Empty = Empty
set path val (Node t l r)
  | (walk path (Node t l r)==Nothing) = (Node t l r)
  | path==[] = (Node val l r)
  | otherwise = help (head path) (tail path) val (Node t l r)
    where 
      help :: Eq a=>Step -> [Step] ->a -> Tree a-> Tree a
      help StepL path val (Node t l r) = (Node t (set path val l) r)
      help StepR path val (Node t l r) = (Node t l (set path val r))
------------------------------------------------------------------------------
...
search val (Node t l r) 
  | t==val = Just []
  | otherwise = if search val l /= Nothing then Just (StepL:steps_1) else if search val r /= Nothing  then Just (StepR:(fromMaybe [] (search val r))) else Nothing
  where 
    steps_1 :: [Step]
    steps_1 = case search val l of
      Just path -> path 
      Nothing -> []  
    steps_2 :: [Step]
    steps_2 = case search val r of 
      Just path -> path
      Nothing -> []
