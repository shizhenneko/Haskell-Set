...
ifM opBool opThen opElse =
  opBool>>= \x ->
    if x==True then opThen else opElse
...
mapM2 op _ [] = return []
mapM2 op xs ys = do
  val <- op (head xs) (head ys)
  rest <- mapM2 op (tail xs) (tail ys)
  return (val:rest)
...
visit maze place = modify (f place)
  where
    f::String->[String]->[String]
    f place z 
      | elem place z = z
      | otherwise = let Just xs = lookup place maze in help f (place:z) xs
        where
          help::(String->[String]->[String])->[String]->[String]->[String]
          help f m [] = m
          help f m (x:xs) = help f (f x m) xs
...
path maze place1 place2 = elem place2 reachable
  where
    reachable::[String]
    reachable=snd $ runState (visit maze place1) []
...
f2 acc x = do
  ...
  let 
      sum :: [Int]->Int
      sum [] = 0
      sum (x:xs) = x+(sum xs) in 
    if elem x curr then return (sum curr) else 
      do
        put (x:curr) 
        return (sum (x:curr))
...
instance Functor SL where
  ...
  fmap g (SL f) = SL h
    where
      h s0 = let (a,b,c)= f s0 in (g a,b,c)
...
instance Monad SL where
  -- implement return and >>=
  return x = SL (\s -> (x,s,[]))
  --  f is (Int -> (a,Int,[String])), g is a->SL b
  (>>=) (SL f) g = SL h
    where
      h s0 =  let 
                (a,b,c) = f s0
