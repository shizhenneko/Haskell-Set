...
buyOneMore (MkShoppingEntry str x y) = MkShoppingEntry str x (y+1)
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
-- add the element of the list and get the average
rgb (Mix a b) = helper (rgb a) (rgb b)
  where 
    helper :: [Double] -> [Double] -> [Double]
    helper [] [] = []
    helper (x:xs) (y:ys) = ((x+y) / 2) : helper xs ys
-- use [1,1,1] to subtract the elem of the rgb x
rgb (Invert x) = helper' (rgb x) 1 
  where 
    helper' :: [Double] -> Double -> [Double]
    helper' [] x = []
    helper' (x:xs) y = (y-x) : helper' xs y
...
toNat :: Int -> Maybe Nat
toNat x =if x>=0 then Just (helper x) else Nothing
helper :: Int -> Nat 
helper 0 = Zero
...
prettyPrint x = reverse (helper x)
  where
    helper :: Bin->String
    helper End = ""
    helper (I x) = '1':helper x 
    helper (O x) = '0':helper x
...
toBin :: Int -> Bin
toBin 0 = O End
toBin x = helper x End
  where 
    helper :: Int -> Bin -> Bin
    helper x m
      | x==0 = m
      | (mod x 2) ==0 = O (helper (div x 2) m)
      | otherwise = I (helper (div x 2) m)
