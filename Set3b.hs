...
sums i = helper 1 i
helper :: Int -> Int -> [Int]
helper m a = case (a==m) of True ->[sum a]
                            False ->(sum m):(helper (m+1) a)
...
mymaximum bigger initial [x] = 
    case (bigger initial x) of True -> initial
                               False -> x
mymaximum bigger initial (x:xs) = 
    case (bigger initial x) of True -> mymaximum bigger initial xs
...
maybeMap f [] = []
maybeMap f (x:xs) = 
    case (f x) of Just a -> a: maybeMap f xs
