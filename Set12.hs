...
fmap2 g x = fmap (fmap g) x

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 g x = fmap (fmap2 g) x
...
count x y = foldr' (help x) 0 y
  where
    help::(Eq a)=>a->a->Int->Int
    help x z m= if z==x then m+1 else m
...
instance Foldable List where
  foldr f z Empty = z
  foldr f z (LNode x y)=f x (foldr f z y)
