...
outputInInput input output = forAll (elements output) (\(x,y) -> y==(length $ filter (== x) input))
...
genList = do
  length<- choose (3,5)
  tense<-(vectorOf length (elements [0..10]))
  return (sort tense)
...
instance Arbitrary Arg where
  arbitrary = do
    numbers<- elements [0..9]
    variable<- elements "abcxyz"
    oneof [return (Number numbers),return (Variable variable)]
