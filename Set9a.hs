...
sumSuccess x = foldr help (Left "no data") x 
  where 
    help :: Either String Int -> Either String Int ->Either String Int
    help (Right x) (Right y) = Right (x+y)
    help (Left x) (Right y) = Right y
    help (Left x) (Left y) = Left y
    help (Right x) (Left y) = Right x
...
data Lock = Lock String | Open String
  deriving Show
...
changeCode str (Lock x) = Lock x
changeCode str (Open x) = Open str
...
permute p q = snd $ unzip (sortBy (comparing fst) (zip p q))
