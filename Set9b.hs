...
type Size = Int
data Chess = Chess Size (Coord -> Char)
prettyPrint :: Size -> [Coord] -> String
prettyPrint n x = invert (Chess n (help x))
    where 
        help :: [Coord]->(Coord->Char)
        help [] = \(x,y)->'.'
        help ((x,y):xs) = combine (\(a,b)-> if a==x&&b==y then 'Q' else '.') (help xs)
            where
                combine :: (Coord -> Char)->(Coord -> Char)->(Coord -> Char)
                combine f g = \x-> if f x == 'Q' || g x=='Q'  then 'Q' else '.'
        invert::Chess -> String
        invert (Chess n f) = help' f n (1,1) 
            where 
                help' :: (Coord->Char)->Size->Coord->String
                help' f n (a,b) 
                 | a<=n && b<n = f (a,b):(help' f n (nextCol (a,b)))
                 | a<n && b==n = f (a,b):'\n':(help' f n (nextRow (a,b)))
                 | a==n && b==n = f (a,b):'\n':""
...
sameDiag (i,j) (k,l) = if (k-i) == (l-j) then True else False

sameAntidiag :: Coord -> Coord -> Bool
sameAntidiag (i,j) (k,l) = if (i-k) == (l-j) then True else False
...
danger queen stack = not (null [x | x<-stack, sameRow queen x||sameCol queen x||sameDiag queen x||sameAntidiag queen x])
...
prettyPrint2 n x = invert (Chess n (help x))
    where
        help :: Stack -> (Coord -> Char)
        help [] = \x->'.'
        help (x:xs) = combine (func x) (help xs)
            where
                combine :: (Coord -> Char)->(Coord -> Char)->(Coord -> Char)
                combine f g coord
                 | f coord == 'Q' || g coord =='Q' = 'Q'
                 | f coord == '#' || g coord == '#' = '#'
                 | otherwise = '.'
                func :: Coord->(Coord->Char)
                func x y =if x==y then 'Q' else if danger x [y] then '#' else '.'
        invert::Chess -> String
        invert (Chess n f) = help' f n (1,1) 
            where 
                help' :: (Coord->Char)->Size->Coord->String
...
fixFirst n [x] = if fst x >n || snd x >n then Nothing else Just [x]
fixFirst n (x:xs)
 | (fst x)>n || (snd x)>n =Nothing
 | danger x xs = fixFirst n ((nextCol x):xs)
 | otherwise = help (x) (fixFirst n xs)
    where
        help :: Coord -> Maybe Stack -> Maybe Stack
        help _ Nothing = Nothing
        help x (Just y) = Just (x:y)
--------------------------------------------------------------------------------
...
step n x 
    | fixFirst n x == Nothing = backtrack x
    | otherwise = continue value
        where
            value :: Stack
            Just value = fixFirst n x
