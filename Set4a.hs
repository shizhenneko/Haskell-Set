...
rangeOf xs = (last value) - (head value)
    where value=(sort xs)
...
longest :: Ord a => [[a]] -> [a]
longest xs = foldr helper [] xs
    where
        helper :: Ord a=>[a] -> [a] -> [a]
        helper cur xs 
            | length xs > length cur = xs
            | length xs == length cur && head xs < head cur = xs
            | otherwise = cur
------------------------------------------------------------------------------
...
incrementKey k [(x,y)] = if k==x then [(x,(y+1))] else [(x,y)]
incrementKey k ((x,y):xs) = if k==x then (x,(y+1)):incrementKey k xs else (x,y):incrementKey k xs
...
helper x map = case Map.lookup x map of
    Nothing -> Map.insert x 1 map
    Just count -> Map.insert x (count + 1) map
...
swap i j arr = arr // (val arr)
    where 
        val arr = val_1 arr : val_2 arr : []
        val_1 arr= (i , arr ! j)
        val_2 arr = (j , arr ! i)
...
maxIndex array = fst (foldr comp valv (Data.Array.assocs array))
    where 
        valv = head (Data.Array.assocs array)

comp :: (Ix i, Ord a) =>(i,a)->(i,a)->(i,a)
comp (i,a) (j,b) = if (a>b) then (i,a) else (j,b)
