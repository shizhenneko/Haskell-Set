...
greetText x 
    | T.length x <=15 = (T.pack "Hello, ")<>x<>(T.pack "!")
    | otherwise = (T.pack "Hello, ")<>(T.take 15 x)<>(T.pack "...!")
...
shout :: T.Text -> T.Text
shout x = T.unwords $ (help 0 (T.words x))
    where
        help :: Int->[T.Text]->[T.Text]
        help _ [] = []
        help i (x:xs) = if even i then (T.toUpper x):(help (i+1) xs) else x:(help (i+1) xs)
...
xorChecksum x 
    | x==(B.pack []) = 0
    | otherwise = xor (B.head x) (xorChecksum (B.tail x))
...
pingpong bs = BL.cycle (BL.fromStrict (B.append bs (B.reverse bs)))
