...
firstHalf xs = if (even $ length xs) then take ((length xs) `div` 2) xs else take (((length xs) `div` 2)+1) xs
palindrome :: String -> Bool
palindrome xs 
    | length xs <= 1 = True
    | otherwise      = if (head xs)==(last xs) then (palindrome ((tail.init)  xs)) else False
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (\x -> x<=max) (pow k 0)
    where 
        pow :: Int -> Int -> [Int]    
        pow k n = k^n : (pow k (n+1))
...
sumRights [] = 0
sumRights (x:xs) = 
    case x of Right a -> a + sumRights xs 
              Left _  -> sumRights xs
...
multiCompose [] xs = xs
multiCompose fs xs = multiCompose (init fs) (last fs $ xs)
