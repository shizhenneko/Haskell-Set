...
blendColor (Color x y z) (Color a b c) = Color (div (x+a) 2) (div (y+b) 2) (div (z+c) 2)

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine f (Picture g) (Picture h) = Picture help
  where 
    help :: Coord -> Color 
    help x = f (g x) (h x)
------------------------------------------------------------------------------
...
rectangle x0 y0 w h = Shape help
  where
    help :: Coord -> Bool
    help (Coord x y) 
...
union (Shape f) (Shape g) = Shape help
  where 
    help :: Coord -> Bool
    help (Coord x y) 
      | f (Coord x y) || g (Coord x y) = True
      | otherwise = False

cut :: Shape -> Shape -> Shape
cut (Shape f) (Shape g) = Shape help
  where 
    help :: Coord -> Bool
    help (Coord x y)
      | f (Coord x y) && (not $ g $ Coord x y) = True
      | otherwise = False
------------------------------------------------------------------------------
...
paintSolid color (Shape shape) (Picture base) = Picture help
  where
    help :: Coord -> Color
    help (Coord x y) 
      | shape (Coord x y) == True = color
...
paint (Picture pat) (Shape shape) (Picture base) = Picture help
  where
    help :: Coord -> Color
    help x
      | shape x == True = pat x
...
instance Transform Flip where
  apply FlipX (Picture z) = Picture help
    where 
      help :: Coord -> Color
      help (Coord x y) = z (Coord (-x) y)
  apply FlipY (Picture z) = Picture help
    where
      help :: Coord -> Color
      help (Coord x y) = z (Coord x (-y))
  apply FlipXY y = flipXY y
------------------------------------------------------------------------------
...
instance (Transform a,Transform b) => Transform (Chain a b) where
  apply (Chain a b) y = apply a $ apply b y
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture func) = Picture help
    where
      help :: Coord -> Color
      help (Coord x y) = Color (div x1 5) (div y1 5) (div z1 5)
        where
          Color x1 y1 z1 = func (Coord x y) <> func  (Coord (x-1) y) <> func  (Coord (x+1) y) <> func  (Coord x (y-1)) <> func  (Coord x (y+1))
...
instance Transform BlurMany where
  apply (BlurMany 1) z = apply Blur z
  apply (BlurMany m) z = apply (BlurMany (m-1)) (apply Blur z)
------------------------------------------------------------------------------
