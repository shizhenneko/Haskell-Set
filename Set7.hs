...
velocity x y= Velocity (val_x / val_y)
  where 
    (Distance val_x)=x
    (Time val_y)=y
...
travel x y = Distance (val_x * val_y)
  where
    (Velocity val_x) = x
    (Time val_y) = y
...
add val (Set (x:xs))
  | val < x = Set (val:x:xs)
  | val == x = Set (x:xs)
  | otherwise = Set (x: unset (add val (Set xs)))
  where 
    unset :: Set a-> [a]
    unset (Set ys) = ys
------------------------------------------------------------------------------
...
data State = Start | Error | Finished | Egg | Flour | Sugar | Flour_And_Sugar | Mixed
  deriving (Eq,Show)
step :: State -> Event -> State
step Start AddEggs = Egg
step Start _ = Error
step Egg AddFlour = Flour
step Egg AddSugar = Sugar
step Egg _ = Error
step Flour AddSugar = Flour_And_Sugar
step Sugar AddFlour = Flour_And_Sugar
step Flour _ = Error
step Sugar _ = Error
step Flour_And_Sugar Mix = Mixed
step Flour_And_Sugar _ = Error
...
average x = sum x / length x
  where
    length :: Fractional a=>NonEmpty a -> a
    length (x:| []) = 1.0
    length (x:| y:ys) = 1.0 + length (y:| ys) 
    sum :: Fractional a=> NonEmpty a -> a
    sum (x:| []) = x
...
reverseNonEmpty x=convert (reverse $ help x)
  where
    help :: NonEmpty a-> [a]
    help (x:| []) = [x]
    help (x:| y:ys) = x:help (y:| ys) 
    convert :: [a] -> NonEmpty a
    convert (x:xs) = x:|xs
...
passwordAllowed x (ContainsSome ss) = any (`elem` x) ss
passwordAllowed x (DoesNotContain y)=not (passwordAllowed x (ContainsSome y))
